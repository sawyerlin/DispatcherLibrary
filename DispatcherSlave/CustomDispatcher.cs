...
namespace DispatcherSlave
{
    internal class CustomDispatcher : Dispatcher
    {
            ...
            : base(pools, cache, producername)
        {
            ...
            _heartBeat = new HeartBeat(neighbours, "192.168.102.118", 8013, 1, 10);
        ...
        public override void Dispatch()
        {
            while (true)
            {
                if (_heartBeat.IsMaster)
                {
                    Job job = GetJob();
                    if (job != null)
                        Console.WriteLine(job + " from dispatcher 8013");
                    else
                        Console.WriteLine("Job is null from dispatcher 8013");
                    Thread.Sleep(1000);
                }
            }
        }
